<template>
  <div style="width: 100%; height: 100%">
    <!-- 加载动画层 -->
    <div v-if="loading" class="transition-loader" :style="loaderStyle">
      <div class="loader-stage">
        <div class="loader-disc" :class="{ 'is-loaded': preloadReady }">
          <div class="disc-grooves"></div>
          <div class="disc-label"><span class="disc-hole"></span></div>
        </div>
        <p class="loader-title">WARP DRIVE ENGAGED</p>
        <p class="loader-target">{{ title || "星际航道" }}</p>
        <div class="loader-track">
          <div
            class="loader-fill"
            :style="{ transform: `scaleX(${progress / 100})` }"
          ></div>
        </div>
        <p class="loader-percent">{{ Math.floor(progress) }}%</p>
        <p v-if="progress >= 100" class="loader-hint">
          {{ statusText }}
        </p>
      </div>
    </div>

    <!-- 页面内容（加载完成后显示） -->
    <div v-show="!loading" style="width: 100%; height: 100%">
      <slot />
    </div>
  </div>
</template>

<script setup>
import { ref, computed, onMounted, provide } from "vue";
import { useRoute } from "vue-router";
import { asset } from "../utils/asset";

const route = useRoute();

const props = defineProps({
  title: { type: String, default: "星际航道" },
  images: { type: Array, default: () => [] },
  models: { type: Array, default: () => [] },
  minDuration: { type: Number, default: 800 },
  skipAnimation: { type: Boolean, default: false },
});

const shouldSkipAnimation = computed(() => {
  return props.skipAnimation || route.query.skipPagePreload === "true";
});

const progress = ref(0);
const loading = ref(!shouldSkipAnimation.value);
const preloadReady = ref(false);
const pageFullyReady = ref(false);
const statusText = ref("正在准备...");

const MIN_ANIMATION_MS = props.minDuration;
const MAX_WAIT_MS = 20000;

provide("setPageReady", () => {
  if (!pageFullyReady.value) {
    pageFullyReady.value = true;
    console.log("[PagePreloader] 子页面就绪信号已收到:", props.title);
  }
});

const preloadImage = (src) => {
  return new Promise((resolve) => {
    if (!src) return resolve();
    const img = new Image();
    img.onload = () => resolve();
    img.onerror = () => {
      console.warn("[PagePreloader] 图片预加载失败:", src);
      resolve();
    };
    img.src = src;
  });
};

const preloadImages = async (paths) => {
  if (!paths || paths.length === 0) return;
  const urls = paths.map((p) => asset(p));
  console.log("[PagePreloader] 预加载图片:", urls);
  await Promise.all(urls.map((url) => preloadImage(url)));
};

const preloadModel = async (url) => {
  if (!url) return;
  try {
    console.log("[PagePreloader] 预加载模型:", url);
    const response = await fetch(url, { cache: "force-cache" });
    if (response.ok) {
      const blob = await response.blob();
      console.log(
        `[PagePreloader] 模型预加载完成: ${(blob.size / 1024 / 1024).toFixed(1)}MB`,
      );
    }
  } catch (err) {
    console.warn("[PagePreloader] 模型预加载失败:", url, err);
  }
};

const preloadModels = async (urls) => {
  if (!urls || urls.length === 0) return;
  await Promise.all(urls.map((url) => preloadModel(url)));
};

const loaderStyle = computed(() => ({
  position: "fixed",
  top: 0,
  left: 0,
  right: 0,
  bottom: 0,
  backgroundColor: "#ffffff",
  zIndex: 99999,
  display: "flex",
  alignItems: "center",
  justifyContent: "center",
  opacity: loading.value ? 1 : 0,
  transition: "opacity 0.5s ease-out",
  pointerEvents: loading.value ? "auto" : "none",
  mixBlendMode: "normal",
}));

const hideLoading = () => {
  if (!loading.value) return;
  statusText.value = "准备就绪";
  setTimeout(() => {
    loading.value = false;
    console.log("[PagePreloader] 加载完成，显示页面:", props.title);
    // emit 事件：通知子页面内容已可见，可重新计算布局/尺寸
    emit("content-visible");
  }, 150);
};

const emit = defineEmits(["content-visible"]);

onMounted(async () => {
  if (shouldSkipAnimation.value) {
    console.log("[PagePreloader] 跳过动画模式，等待子页面就绪");
    try {
      await Promise.all([
        preloadImages(props.images),
        preloadModels(props.models),
      ]);
      preloadReady.value = true;
      console.log("[PagePreloader] 后台预加载完成:", props.title);
    } catch (err) {
      console.error("[PagePreloader] 后台预加载失败:", err);
      preloadReady.value = true;
    }

    const checkInterval = setInterval(() => {
      if (pageFullyReady.value) {
        clearInterval(checkInterval);
        loading.value = false;
        console.log("[PagePreloader] 子页面就绪，显示内容");
        emit("content-visible");
      }
    }, 100);

    setTimeout(() => {
      clearInterval(checkInterval);
      if (loading.value) {
        console.warn("[PagePreloader] 超时 (skip模式)，强制显示");
        loading.value = false;
        emit("content-visible");
      }
    }, MAX_WAIT_MS);

    return;
  }

  const startTime = performance.now();
  let animationFrameId = null;

  const animate = (currentTime) => {
    const elapsed = currentTime - startTime;
    const timeProgress = Math.min(100, (elapsed / MIN_ANIMATION_MS) * 100);

    if (preloadReady.value && pageFullyReady.value) {
      progress.value = Math.min(progress.value + 8, 100);
      if (progress.value >= 100) {
        progress.value = 100;
        hideLoading();
        return;
      }
    } else if (preloadReady.value && elapsed < MIN_ANIMATION_MS) {
      progress.value = Math.min(timeProgress + (100 - timeProgress) * 0.6, 95);
    } else if (preloadReady.value) {
      progress.value = Math.max(progress.value, 95);
    } else {
      progress.value = Math.min(timeProgress * 0.5, 90);
    }

    if (loading.value) {
      animationFrameId = requestAnimationFrame(animate);
    }
  };

  animationFrameId = requestAnimationFrame(animate);

  statusText.value = "正在加载资源...";
  try {
    await Promise.all([
      preloadImages(props.images),
      preloadModels(props.models),
    ]);
    preloadReady.value = true;
    statusText.value = "正在准备页面...";
    console.log("[PagePreloader] COS资源预加载完成:", props.title);
  } catch (err) {
    console.error("[PagePreloader] 预加载失败:", err);
    preloadReady.value = true;
    statusText.value = "正在校准航线...";
  }

  const checkInterval = setInterval(() => {
    if (pageFullyReady.value) clearInterval(checkInterval);
  }, 100);

  setTimeout(() => {
    clearInterval(checkInterval);
    if (animationFrameId) cancelAnimationFrame(animationFrameId);
    if (loading.value) {
      console.warn("[PagePreloader] 超时保护，强制显示内容");
      preloadReady.value = true;
      pageFullyReady.value = true;
      progress.value = 100;
      hideLoading();
    }
  }, MAX_WAIT_MS);
});
</script>

<style scoped>
.loader-stage {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 20px;
}

.loader-disc {
  width: 120px;
  height: 120px;
  border-radius: 50%;
  background: #111;
  position: relative;
  animation: spin 2s linear infinite;
  box-shadow: 0 10px 40px rgba(0, 0, 0, 0.15);
  transition:
    box-shadow 0.4s ease,
    background 0.4s ease;
}

.loader-disc.is-loaded {
  box-shadow: 0 10px 40px rgba(212, 175, 55, 0.35);
}

.disc-grooves {
  position: absolute;
  inset: 8px;
  border-radius: 50%;
  border: 1px solid rgba(255, 255, 255, 0.08);
  background: repeating-radial-gradient(
    circle at center,
    transparent 0,
    transparent 2px,
    rgba(255, 255, 255, 0.06) 3px,
    transparent 4px
  );
}

.disc-label {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  width: 36px;
  height: 36px;
  border-radius: 50%;
  background: #d4af37;
  display: flex;
  align-items: center;
  justify-content: center;
}

.disc-hole {
  width: 6px;
  height: 6px;
  border-radius: 50%;
  background: #fff;
}

@keyframes spin {
  from {
    transform: rotate(0deg);
  }
  to {
    transform: rotate(360deg);
  }
}

.loader-title {
  font-size: 12px;
  letter-spacing: 6px;
  color: #999;
  text-transform: uppercase;
  margin: 0;
}

.loader-target {
  font-size: 20px;
  font-weight: 600;
  letter-spacing: 4px;
  color: #111;
  margin: 0;
}

.loader-track {
  width: 200px;
  height: 2px;
  background: #eee;
  border-radius: 1px;
  overflow: hidden;
  margin-top: 4px;
}

.loader-fill {
  width: 100%;
  height: 100%;
  background: #111;
  border-radius: 1px;
  transform-origin: left;
  transition: transform 0.1s ease-out;
}

.loader-percent {
  font-size: 11px;
  letter-spacing: 2px;
  color: #aaa;
  margin: 0;
}

.loader-hint {
  font-size: 11px;
  letter-spacing: 2px;
  color: #ccc;
  margin: 0;
  animation: pulse 1.5s ease infinite;
}

@keyframes pulse {
  0%,
  100% {
    opacity: 0.4;
  }
  50% {
    opacity: 1;
  }
}

:global(body.transition-loader-active) {
  background: #ffffff !important;
  background-color: #ffffff !important;
}
</style>
